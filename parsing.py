...
class Generator():
    def __init__(self, pageNumber, companylink) -> None:
...
class Parser():
    def __init__(self, html_context: str, isBlog: bool = True, *args, **kwargs) -> None:
...
async def worker(i):
    ...
    html_context = await get_html(url=companies_url)
    all_arcticles_href = Parser(html_context=html_context, isBlog=False).find_arcticles()

    pool = Pool(10)
    pool.map_async(GodClass.parseCompanyData, all_arcticles_href)

if __name__ == "__main__":
    pool = Pool(2)
    pool.map_async(worker, range(1,17))